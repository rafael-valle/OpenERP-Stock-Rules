...
class stock_rules(osv.osv):
    ...
    _columns = {
        ...
        'category_id': fields.many2one('product.category', 'Product category', help="leave blank to apply to all products"),
        'stock_warehouse_orderpoint_id': fields.one2many('stock.warehouse.orderpoint','stock_rule_id', 'Warehouse orderpoint'),
    ...
    def generate_rules(self, cr, uid, ids, context=None):
        ...
        if stock_rules_obj.category_id:
            ...
            if stock_rules_obj.child_categories == False:
                ...
                products_ids=self.pool.get('product.product').search(cr,uid,[('type','=','product'),('procure_method','=','make_to_stock'),('categ_id','=',stock_rules_obj.category_id.id)])
                ...
                products_ids=self.pool.get('product.product').search(cr,uid,[('type','=','product'),('procure_method','=','make_to_stock'),('categ_id','in',childs)])
        ...
        # if not category
            ...
            products_ids=self.pool.get('product.product').search(cr,uid,[('type','=','product'),('procure_method','=','make_to_stock')])
